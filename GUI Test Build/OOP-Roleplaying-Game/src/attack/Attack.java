...
public class  Attack {
	...
	public String toString() {
			return "The attack " + name + " of type " + type + " deals "+ damage + " damage to "+target;
		
	}
	
	public void attackEnemy(Player p , Enemy enemy) {
		for(int i = 0; i < target.size(); i++) {
			Random rand = new Random();
			int l = rand.nextInt(5);
			if(p.getStamina()<=0){
				if(target.get(i).equals(Target.values()[1])) {
					p.setStamina(p.getStamina() - damage);
				}
				else
					System.out.println("You can only use the rest button");
